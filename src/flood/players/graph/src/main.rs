...
impl GraphSinglePlayerSolver {
    ...
    fn solve(&mut self) -> usize {
        ...
        loop {
            ...
            match self._solve(&initial_flooded, &mut vec![]) {
                Some(solution) => {
                    self.print_speed();
                    ...
                    best_move = Some(solution.moves[0]);
                }
                None => break,
            }
        }
    ...
    fn _solve(&mut self, flooded: &BitSet, moves: &mut Vec<usize>) -> Option<Solution> {
        if moves.len() > self.max_moves {
            return None;
        }

        if self.count_unflooded_colors(flooded) + moves.len() > self.max_moves {
            return None;
        }
        ...
        for (mov, _, newly_flooded) in move_tuples {
            ...
            if let Some(solution) = self._solve(&child_flooded, &mut child_moves) {
                return Some(solution);
            }
        }
...
fn main() -> Result<()> {
    ...
    let best_move = solver.solve();
    println!("Solution: {}", best_move);
